"""
schema_reader.py — Updated with Aditi's REAL table and column names.

Aditi's actual tables (from her CSV files):
  customers   → customer_id, country, signup_date
  products    → product_id, product_name, category, price
  orders      → order_id, customer_id, order_date, status
  order_items → order_id, product_id, quantity, price

NOTE: There is NO 'sales' table and NO 'name' column in customers.
      Aditi uses 'order_items' (not 'sales') and customers only have country.
"""
...
def _get_live_schema() -> str:
    try:
        ...
        conn = psycopg2.connect(
            ...
            user="postgres", password="123QWEzx@"
        )
        cur = conn.cursor()
        cur.execute("""
